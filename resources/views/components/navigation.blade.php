        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="flex-shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="text-xl font-bold text-blue-600 hover:text-blue-700 transition">
                        {{ config('app.name') }}
                    </a>
                </div>
                        ...
                        <div class="relative group">
                            <a href="{{ route($item['route']) }}"
                               class="inline-flex items-center px-3 py-2 text-sm font-medium 
                                      {{ request()->routeIs($key.'.*') ? 'text-blue-600 border-b-2 border-blue-500' : 'text-gray-500 hover:text-gray-700 hover:border-gray-300 border-b-2 border-transparent' }}
                            ...
                            @if(array_filter($item['actions']))
                                <div class="absolute z-10 hidden group-hover:block mt-1 w-48">
                                    <div class="py-1 bg-white rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                                        @if($item['actions']['create'])
                                            <a href="{{ route($key.'.create') }}"
                                               class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
                                                Создать {{ mb_strtolower($item['name']) }}
                                            </a>
                                        @endif
                                    </div>
                                </div>
